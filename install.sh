...
# AetherOS fica exposto na porta 80 do host (ver docker-compose.yml), então
# dá pra acessar só com http://<ip>, sem precisar digitar a porta.
PORT="80"
...
if command -v ss &>/dev/null && ss -ltn 2>/dev/null | grep -q ":${PORT} "; then
    Show 3 "port ${PORT} looks like it's already in use by something else on this machine (another web server, maybe?). AetherOS might fail to start — free up port ${PORT} or edit the port mapping in docker-compose.yml."
fi
